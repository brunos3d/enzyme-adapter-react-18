...
function transformSuspense(renderedEl, prerenderEl, { suspenseFallback }) {
  ...
  const {
    propTypes, defaultProps, contextTypes, contextType, childContextTypes,
  } = renderedEl.type;
  ...
  const FakeSuspense = Object.assign(
    isStateful(prerenderEl.type)
      ...
      : (props) =>
        ...
        React.createElement(renderedEl.type, { ...renderedEl.props, ...props }, children),
    {
      propTypes,
      defaultProps,
      contextTypes,
...
function isStateful(Component) {
  return (
    Component.prototype && (Component.prototype.isReactComponent || Array.isArray(Component.__reactAutoBindPairs)) // fallback for createClass components
  );
}
  ...
  createMountRenderer(options) {
    ...
    return {
      render(el, context, callback) {
        return wrapAct(() => {
          if (instance === null) {
            ...
            const ReactWrapperComponent = createMountWrapper(el, {
              ...options,
              adapter,
            });
            const wrappedEl = React.createElement(ReactWrapperComponent, wrapperProps);
            instance = hydrateIn ? ReactDOM.hydrate(wrappedEl, domNode) : ReactDOM.render(wrappedEl, domNode);
      ...
      getNode() {
        ...
        return getNodeFromRootFinder(adapter.isCustomComponent, toTree(instance._reactInternals), options);
      },
      simulateError(nodeHierarchy, rootNode, error) {
        ...
        simulateError(error, catchingInstance, rootNode, nodeHierarchy, nodeTypeFromType, adapter.displayNameOfNode, catchingType);
      },
  ...
  createShallowRenderer(options = {}) {
      ...
      if (lastComponent !== Component) {
        if (isStateful(Component)) {
          ...
          wrappedComponent = function (props, ...args) {
            const shouldUpdate = memoized === sentinel || (compare ? !compare(prevProps, props) : !shallowEqual(prevProps, props));
            if (shouldUpdate) {
    ...
    const renderElement = (elConfig, ...rest) => {
      ...
      if (renderedEl && renderedEl.type) {
        const clonedEl = transformSuspense(renderedEl, elConfig, {
          suspenseFallback,
        });
    ...
    return {
      render(el, unmaskedContext, { providerValues = new Map() } = {}) {
        ...
        if (typeof el.type === 'string') {
          ...
          if (isMemo(el.type)) {
            ...
            return withSetStateAllowed(() => renderElement({ ...el, type: wrapPureComponent(InnerComp, compare) }, context));
          }

          if (!isStateful(Component) && typeof Component === 'function') {
            return withSetStateAllowed(() => renderElement({ ...renderedEl, type: Component }, context));
          }
      ...
      checkPropTypes(typeSpecs, values, location, hierarchy) {
        return checkPropTypes(typeSpecs, values, location, displayNameOfNode(cachedNode), () => getComponentStack(hierarchy.concat([cachedNode])));
      },
    };
  }
